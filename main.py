...
def bucle_monitoreo(switch_ref, texto_estado, page):
    ...
    while switch_ref.value:
        ...
        # Solo manda reporte si es la primera vez O si la IP cambió
        if ip_actual != ultima_ip_reportada:
            payload = {
            ...
            try:
                ...
                texto_estado.value = f"Reportado: {ip_actual}"
                ultima_ip_reportada = ip_actual  # Actualizamos la IP guardada
            except Exception:
                texto_estado.value = "Estado: Error de conexión"
            
            page.update()
        
        # Espera de 10 segundos para revisar la IP rápido sin saturar la red
        for _ in range(10):
